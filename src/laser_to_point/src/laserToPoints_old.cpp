...
void poseCallback (const geometry_msgs::PoseStamped::ConstPtr& pose_in, LaserScanToPointCloud* l)
{
    //std::cout<<pose_in->pose.position.x <<std::endl;
    geometry_msgs::PoseStamped orig = *pose_in;
    float tmp = orig.pose.position.x;
    orig.pose.position.x = orig.pose.position.z;
    orig.pose.position.z = tmp;
    orig.pose.position.y = -orig.pose.position.y;

    l->currentPose = pose_in->pose;
    tf::Quaternion q_rot, q_orig, q_new;
    double r=3.14159/2, p=0, y=0;
    q_rot = tf::createQuaternionFromRPY(r, p, y);
    quaternionMsgToTF(l->currentPose.orientation , q_orig);  // Get the original orientation of 'commanded_pose'
...
int main(int argc, char** argv)
{
    ...
    lstopc.pose_sub_ = n.subscribe<geometry_msgs::PoseStamped>("/vrpn_client_node/drone_1/pose", 100, boost::bind(poseCallback, _1, &lstopc));
    pub = n.advertise<geometry_msgs::PoseStamped>("repose", 1000);
